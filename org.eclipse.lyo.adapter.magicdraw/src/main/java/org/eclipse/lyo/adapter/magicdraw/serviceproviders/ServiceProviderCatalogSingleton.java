...
public class ServiceProviderCatalogSingleton {
	 ...
	 */
	protected static void initServiceProvidersFromProjects(
			HttpServletRequest httpServletRequest) {
		try {
			File directory = new File(
					OSLC4JMagicDrawApplication.magicdrawModelsDirectory);
				...
				if (!serviceProviders.containsKey(magicDrawProjectName)) {
					...
					final ServiceProvider magicdrawServiceProvider = MagicDrawServiceProviderFactory
							.createServiceProvider(basePath,
									magicDrawProjectName, parameterMap);
					registerServiceProvider(basePath, magicdrawServiceProvider,
							magicDrawProjectName);
				}
			}

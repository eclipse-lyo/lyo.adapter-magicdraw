...
package org.eclipse.lyo.adapter.magicdraw.application;
...
import org.eclipse.lyo.adapter.magicdraw.services.OSLC4JMagicDrawApplication;
import org.eclipse.lyo.adapter.magicdraw.stereotypes.MDModelLibException;
import org.eclipse.lyo.adapter.magicdraw.stereotypes.MDSysMLModelHandler;
 ...
 * MagicDraw application and the OSLC MagicDraw adapter.  It is used to 
 * load MagicDraw SysML projects, retrieve SysML elements from projects,
 * and map SysML elements to OSLC resources described as POJOs
 * 
...
//	static Map<String, Class> qNameMdSysmlBlockMap = new HashMap<String, Class>();
	...
	/**
	 * This method retrieves SysML elements from a specific MagicDraw project (mdzip file).
	 * 
	 * This method is invoked by most or all web services of the OSLC MagicDraw adapter.
	 * 
	 *  @param projectId the name of the MagicDraw mdzip file (not the name of the SysML model contained in the mdzip file!)
	 *  
	 */
	...
	private static void mapSysMLValueTypes() {
		for (DataType mdSysMLValueType : mdSysmlValueTypes) {
			...
			try {
				...
				qNameOslcSysmlValueTypeMap.put(magicDrawFileName + "/valuetypes/" + qName.replaceAll("\\n", "-")
						.replaceAll(" ", "_"), sysMLValueType);
	...
	private static void mapSysMLAssociationBlocks() {
		for (com.nomagic.uml2.ext.magicdraw.classes.mdassociationclasses.AssociationClass mdSysMLAssociationBlock : mdSysmlAssociationBlocks) {
			...
			try {
				...
				qNameOslcSysmlAssociationBlockMap.put(magicDrawFileName + "/associationblocks/" + 
						qName.replaceAll("\\n", "-").replaceAll(" ", "_"),
	...
	private static void mapSysMLInterfaceBlocks() throws MDModelLibException {
		for (Class mdSysMLBlock : mdSysmlInterfaceBlocks) {
			...
			try {
				...
				qNameOslcSysmlInterfaceBlockMap.put(magicDrawFileName + "/interfaceblocks/" + qName
						.replaceAll("\\n", "-").replaceAll(" ", "_"),
						sysMLInterfaceBlock);
			...
			if (property.getAppliedStereotypeInstance() != null) {
				...
				if (stereotypeInstance.getClassifier().get(0).getName()
					...
					try {
						...
						qNameOslcSysmlFlowPropertyMap.put(magicDrawFileName + "/flowproperties/" + property
								.getQualifiedName().replaceAll("\\n", "-")
								.replaceAll(" ", "_"), sysmlFlowProperty);
...
//	static Collection<com.nomagic.uml2.ext.magicdraw.classes.mdkernel.Class> getAllSysMLAssociationBlocks(
//			com.nomagic.uml2.ext.magicdraw.classes.mdkernel.PackageableElement packageableElement) throws MDModelLibException {
...
//			for (PackageableElement nestedPackageableElement : package_
//					.getPackagedElement()) {
//				if (MDSysMLModelHandler.isSysMLElement(nestedPackageableElement, "AssociationBlock")) {
...
//				if (MDSysMLModelHandler.isSysMLElement(nestedClassifier, "AssociationBlock")) {
//					sysmlBlocks
...
//	}
	...
	private static void mapSysMLPackageRelationships() {
		for (com.nomagic.uml2.ext.magicdraw.classes.mdkernel.Package mdSysMLPackage : mdSysmlPackages) {
			...
			SysMLPackage sysMLPackage = qNameOslcSysmlPackageMap
					.get(magicDrawFileName + "/packages/" + mdSysMLPackage.getQualifiedName()
							.replaceAll("\\n", "-").replaceAll(" ", "_"));
	...
	private static void mapSysMLPackages() {
		for (com.nomagic.uml2.ext.magicdraw.classes.mdkernel.Package mdSysMLPackage : mdSysmlPackages) {
			...
			try {
				...
				qNameOslcSysmlPackageMap.put(magicDrawFileName + "/packages/" + qName.replaceAll("\\n", "-")
						.replaceAll(" ", "_"), sysMLPackage);
	...
	private static void mapSysMLBlockRelationships() throws MDModelLibException {
		for (Class mdSysmlBlock : mdSysmlBlocks) {
			SysMLBlock sysmlBlock = qNameOslcSysmlBlockMap.get(magicDrawFileName + "/blocks/" + mdSysmlBlock
					.getQualifiedName().replaceAll("\\n", "-")
					.replaceAll(" ", "_"));
			...
			for (Classifier inheritedclassifier : inheritedClassifiers) {
				...
				SysMLBlock inheritedBlock = qNameOslcSysmlBlockMap
						.get(magicDrawFileName + "/blocks/" + qNameInheritedclassifier.replaceAll("\\n", "-")
								.replaceAll(" ", "_"));

				URI inheritedBlockURI = null;
				try {
					inheritedBlockURI = new URI(baseHTTPURI + "/services/"
							+ projectId + "/blocks/" + qNameInheritedclassifier.replaceAll("\\n", "-")
							.replaceAll(" ", "_"));
			...
			for (Classifier nestedClassifier : nestedClassifiers) {
				...
				SysMLBlock nestedBlock = qNameOslcSysmlBlockMap
						.get(magicDrawFileName + "/blocks/" + qNameNestedclassifier.replaceAll("\\n", "-")
								.replaceAll(" ", "_"));
...
//					mdSysMLBlock);
			...
			try {
				...
				qNameOslcSysmlBlockMap.put(magicDrawFileName + "/blocks/" + qName.replaceAll("\\n", "-")
			...
			if (property.getAppliedStereotypeInstance() != null) {
				...
				if (stereotypeInstance.getClassifier().get(0).getName()
					...
					try {
						...
						qNameOslcSysmlValuePropertyMap.put(magicDrawFileName + "/valueproperties/" +
...
//			} 
		...
		if (portsList.size() > 0) {
			...
			for (Port port : portsList) {
				...
				try {
					...
					qNameOslcSysmlPortMap.put(magicDrawFileName + "/ports/" + getQualifiedNameOrID(port),
							sysMLPort);
			...
			if (property.getAppliedStereotypeInstance() != null) {
				...
				if (stereotypeInstance.getClassifier().get(0).getName()
					...
					try {
						...
						qNameOslcSysmlReferencePropertyMap.put(magicDrawFileName + "/referenceproperties/" + property
								.getQualifiedName().replaceAll("\\n", "-")
								.replaceAll(" ", "_"), sysmlReferenceProperty);
						...
						Association mdSysMAssociation = property
								.getAssociation();
						URI linkedElementURI = null;
						String baseURI = baseHTTPURI + "/services/" + projectId
								+ "/unknown/";
						...
						if (MDSysMLModelHandler.isSysMLElement(property.getAssociation(), "Block")) {
							...
							// .getAssociation());
							// }
							baseURI = baseHTTPURI + "/services/" + projectId
									+ "/associationblocks/";
	...
	private static URI getDirectedLinkSysMLElement(boolean isElementSource,
			Element element, String relationshipType) throws MDModelLibException {
		...
		for (DirectedRelationship directedRelationship : directedRelationships) {
			try{
				String directedRelationshipType = directedRelationship.getAppliedStereotypeInstance().getClassifier().get(0).getName();
	...
	private static Link[] getDirectedLinksOfSysMLElement(
			boolean isElementSource, Element element, String relationshipType) throws MDModelLibException {
			...
			if (property.getAppliedStereotypeInstance() != null) {
				...
				if (stereotypeInstance.getClassifier().get(0).getName()
					...
					try {
						...
						qNameOslcSysmlPartPropertyMap.put(magicDrawFileName + "/partproperties/" +
								getQualifiedNameOrID(property),
								sysmlPartProperty);
...
//			String key = (String) qNameOslcSysmlConnectorEndMap.keySet().toArray()[0];
//			if(key.startsWith("/oslc4jmagicdraw/services/")){
//				propertyQualifiedName = "/oslc4jmagicdraw/services/" + projectId + "/connectorends/" + propertyQualifiedName;
//			}
//		}
	...
	public static void createSysMLPackage(SysMLPackage sysmlPackage,
		...
		if (!elementExists) {
			...
			qNameOslcSysmlPackageMap.put(projectId2 + "/packages/" + getQualifiedNameOrID(newMDPackage),
					sysmlPackage);
	...
	public static void createSysMLBlock(SysMLBlock sysmlBlock, String projectId2) {
		...
		if (!elementExists) {
			...
			qNameOslcSysmlBlockMap.put(projectId2 + "/blocks/" + getQualifiedNameOrID(newMDBlock),
					sysmlBlock);
		...
		if (!elementExists) {
			...
			for (Class mdSysmlBlock : mdSysmlBlocks) {
				if (mdSysmlBlock.getQualifiedName().equals(ownerQualifiedName)) {
					...
					// get the OSLC SysML block and add the OSLC SysML part to it
					SysMLBlock sysmlBlock = qNameOslcSysmlBlockMap.get(projectId2 + "/blocks/" + mdSysmlBlock
							.getQualifiedName().replaceAll("\\n", "-")
							.replaceAll(" ", "_"));					
	...
	public static void createSysMLConnector(SysMLConnector sysmlConnector,
		...
		if (!elementExists) {
			...
			SysMLConnectorEnd sysmlConnectorEnd1 = qNameOslcSysmlConnectorEndMap
					.get(projectId2 + "/connectorends/" + connectorEnd1URI.getRawPath());
			SysMLConnectorEnd sysmlConnectorEnd2 = qNameOslcSysmlConnectorEndMap
					.get(projectId2 + "/connectorends/" + connectorEnd2URI.getRawPath());
	...
	public static void createSysMLConnectorEnd(
		...
		String rawPath = sysmlConnectorEnd.getAbout()
				.getRawPath();
		String sysmlConnectorEndID = rawPath.replace("/oslc4jmagicdraw/services/" + projectId2 + "/connectorends/", "");				
//		qNameOslcSysmlConnectorEndMap.put(sysmlConnectorEndID, sysmlConnectorEnd);
		...
		if (!elementExists) {
			...
			for (Class mdSysmlBlock : mdSysmlBlocks) {
				if (mdSysmlBlock.getQualifiedName().equals(ownerQualifiedName)) {
					...
					// get the OSLC SysML block and add the OSLC SysML port to it
					SysMLBlock sysmlBlock = qNameOslcSysmlBlockMap.get(projectId2 + "/blocks/" + mdSysmlBlock
							.getQualifiedName().replaceAll("\\n", "-")
							.replaceAll(" ", "_"));					
			...
			qNameOslcSysmlPortMap.put(projectId2 + "/ports/" + getQualifiedNameOrID(newMDPort),
					sysmlPort);
	...
	public static URI getURIFromQualifiedName(String typeAndQualifiedName) {
		...
		URI elementURI = URI
				.create(baseHTTPURI + "/services/"
						+ projectId + "/" + elementType + "/" + qualifiedName);
		...
		if (!elementExists) {
			...
			for (Class mdSysmlBlock : mdSysmlBlocks) {
				if (mdSysmlBlock.getQualifiedName().equals(ownerQualifiedName)) {
					...
					// get the OSLC SysML block and add the OSLC SysML value property to it
					SysMLBlock sysmlBlock = qNameOslcSysmlBlockMap.get(projectId2 + "/blocks/" + mdSysmlBlock
							.getQualifiedName().replaceAll("\\n", "-")
							.replaceAll(" ", "_"));					
	...
	public static void createSysMLInterfaceBlock(
		...
		if (!elementExists) {
			...
			qNameOslcSysmlInterfaceBlockMap.put(projectId2 + "/interfaceblocks/" +  
	...
	public static void createSysMLFlowProperty(
		...
		if (!elementExists) {
			...
			qNameOslcSysmlFlowPropertyMap.put(projectId2 + "/flowproperties/" + 
					getQualifiedNameOrID(newMDFlowProperty), sysmlFlowProperty);
		}

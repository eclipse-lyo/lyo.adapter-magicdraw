...
SysMLModel element = (SysMLModel)request.getAttribute("model");
...
<title>MagicDraw OSLC Adapter: MagicDraw Model</title>
...
<body onload="">
	...
	<!-- header -->
	<p id="title">MagicDraw OSLC Adapter: MagicDraw Model</p>
	...
	<div id="main-body">
		...
		<h1><span id="metainfo">MagicDraw Model </span><%= element.getName() %></h1>
		<br>
		...
		<% if( element.getPackages().length > 0) {  %>
		<p><span id="metainfo">Packages</span></p>
		<!-- list representation of resource relationships -->				
			<% for (Link link : element.getPackages()) {  %>				

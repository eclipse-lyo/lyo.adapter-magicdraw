...
SysMLProxyPort element = (SysMLProxyPort)request.getAttribute("proxyPort");
...
<title>MagicDraw OSLC Adapter: MagicDraw Proxy Port</title>
...
<body onload="">
	...
	<!-- header -->
	<p id="title">MagicDraw OSLC Adapter: MagicDraw Proxy Port</p>
	...
	<div id="main-body">
		...
		<h1><span id="metainfo">MagicDraw Proxy Port </span><%= element.getName() %></h1>
		<br>

		<!-- resource attributes and relationships -->
		
		<% if(element.getLower() != null) { %>
			<p><span id="metainfo">Lower Multiplicity: </span><%= element.getLower() %></p>
		<% } %>
		
		<% if(element.getUpper() != null) { %>
			<p><span id="metainfo">Upper Multiplicity: </span><%= element.getUpper() %></p>
		<% } %>
		...
		<p><span id="metainfo">isService: </span><%= element.getIsService() %></p>
		<p><span id="metainfo">isBehavior: </span><%= element.getIsBehavior() %></p>
		<p><span id="metainfo">isConjugated: </span><%= element.getIsConjugated() %></p>

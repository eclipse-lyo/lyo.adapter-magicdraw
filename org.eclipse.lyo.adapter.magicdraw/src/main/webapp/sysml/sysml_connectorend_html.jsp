...
<body onload="">
	...
	<!-- header -->
	<p id="title">MagicDraw OSLC Adapter: MagicDraw Connector End</p>
	...
	<div id="main-body">
		...
		<h1><span id="metainfo">MagicDraw Connector End </span><%= getElementQualifiedName(element.getAbout()) %></h1>
		<br>

		<!-- resource attributes and relationships -->
		
		<% if(element.getRole() != null) { %>
			<p><span id="metainfo">Role: </span><a href="<%= element.getRole() %>"> <%= getElementQualifiedName(element.getRole()) %></a></p>
		<% } %>
		
		<% if(element.getDefiningEnd() != null) { %>
			<p><span id="metainfo">Defining End: </span><a href="<%= element.getDefiningEnd() %>"> <%= getElementQualifiedName(element.getDefiningEnd()) %></a></p>
		<% } %>
		
		<% if(element.getPartWithPort() != null) { %>
			<p><span id="metainfo">Part With Port: </span><a href="<%= element.getPartWithPort() %>"> <%= getElementQualifiedName(element.getPartWithPort()) %></a></p>
		<% } %>

...
SysMLAssociationBlock block = (SysMLAssociationBlock)request.getAttribute("associationblock");
...
<title>MagicDraw OSLC Adapter: MagicDraw Association Block</title>
...
<body onload="">
	...
	<!-- header -->
	<p id="title">MagicDraw OSLC Adapter: MagicDraw Association Block</p>
	...
	<div id="main-body">
		...
		<h1><span id="metainfo">MagicDraw Association Block </span><%= block.getName() %></h1>
		<br>

		<!-- resource attributes and relationships -->
		
		<% if( block.getMemberEnds().length > 0) {  %>
		<p><span id="metainfo">Member Ends</span></p>
		<!-- list representation of resource relationships -->				
			<% for (Link link : block.getMemberEnds()) {  %>				
